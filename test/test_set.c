...
#define set_insert mcc_set_insert
#define set_remove mcc_set_remove
#define set_get mcc_set_get
#define set_clear mcc_set_clear

int main(void)
{
	struct fruit tmp;
	const struct fruit *ref;
	struct mcc_set *set = mcc_set_new(&fruit_);
	assert(set != NULL);
